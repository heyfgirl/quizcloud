...
module.exports =async function (option,base) {
    ...
    try{
        ...
        if(option){
            if(!isEmpty(base)){
                ...
                var dest = path.join(static_url,"admin",filetype,store_file_name);   //  测试使用存储的地方
                ...
                //存储数据
                let FileModel = Models.getTableModel("File");
                ...
                store.address =  "http://admin.quizyun.com/static/" + path.join( "admin/"+filetype,store_file_name).replace(/\\/g,'/');//////临时
