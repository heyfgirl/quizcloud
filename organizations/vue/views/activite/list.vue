...
export default {
    ...
    created() {
        ...
        // 组件创建完后获取数据，
        this.getActiviteList(this.page,this.size,this.kw,);
    },
    //监视变量变化
    watch: {
    ...
    methods: {
        ...
        search(){
            this.page = 1;
            this.getActiviteList(this.page,this.size,this.kw,this.view);
        },
        //列表获取colums
        changeTableColumns(){
            let sho = [];
        ...
        onPageChange(page){
            this.page=page;
            this.getActiviteList(page);
        },
        //页面大小更改
        onSizeChange(size){
            this.size=size;
            this.getActiviteList(this.page,size);
        },
        //获取数据list
        getActiviteList(page,size,kw,filter){
            var params ={};
            params.page = this.page= page || this.page;
            params.kw = kw || this.kw;
            params.size = size || this.size;
            params.filter = filter || this.filter;
            this.$store.dispatch('getActiviteList',params).then((data) => {
